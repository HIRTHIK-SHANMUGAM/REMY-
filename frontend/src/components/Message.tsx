...
function MessageBase({ message }: { message: ChatMessage }) {
  ...
  return (
    <div
      className={`flex animate-fade-in gap-3 ${isUser ? "justify-end" : "justify-start"}`}
    >
      {!isUser && (
      ...
      <div
        className={[
          "max-w-[85%] rounded-2xl px-4 py-2.5 text-[14.5px] leading-relaxed sm:max-w-[75%]",
          isUser
            ? "rounded-br-md bg-accent text-white"
            : "rounded-bl-md border border-surface-3 bg-surface-1 text-ink",
        ].join(" ")}
      >
        {isUser ? (
